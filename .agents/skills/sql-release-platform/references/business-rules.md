...
检查当前日志的完整内容，`COMMIT` 和 `ROLLBACK` 作为独立关键字匹配，且不区分大小写：

- 包含 `ROLLBACK`：失败，显示红色标签。
- 不包含 `ROLLBACK`，但包含 `COMMIT`：成功，显示绿色标签。
- 两者都没有：状态不确定，显示黄色“请检查”标签。
- 同一日志同时包含 `COMMIT` 和 `ROLLBACK` 时，`ROLLBACK` 优先，判定为失败。

状态标签紧跟日志文件名。每个日志卡片标题使用不同颜色，方便同时查看多个数据库。
